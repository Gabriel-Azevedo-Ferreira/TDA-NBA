...
colorWithPlayer <- function(player,map) {
  ...
  for(points in map$points_in_vertex){
    col <- 0
    if(any(points == player)){
      col <- 5
    }
    cWithPlayer <-c(cWithPlayer,col)  
  }
  return(cWithPlayer)
}
...
map    <- mapper2D(
          ...
          num_intervals = c(40,40),#20,20 or 30,30
          percent_overlap = 50,
          num_bins_when_clustering = 30 # Explaining this parameter:
              # The cluster phase here is a hierarquical cluster. to define the
              # number of cluster from the hTree, we splite the real interval 
              # [0,max(height)] into num_bins_... intervals of the same size (that is 
              # if num_b... is 2, 2 intervals will be created, [0, max(height)/2) and 
...
MapperNodes$Nodesize <- (150)*(MapperNodes$Nodesize)
...
#colScale <- 'd3.scaleLinear().domain([0, 30.11]).range(["black", "blue"]);'
#MapperNodes$Nodegroup <- meanforcolor(map = map,data = df)

#color according to a specific player
colScale <- "d3.scaleOrdinal(d3.schemeCategory20);"
MapperNodes$Nodegroup <- colorWithPlayer(player = 291,map)
...
forceNetwork(Nodes = MapperNodes, Links = MapperLinks, 
            ...
            fontSize = 50,
            colourScale = JS(colScale), zoom = T) %>% saveNetwork(file = 'Net1.html')
